...
def check_text_length( f, filename, min_chars, max_chars):
    ...
    for line in f:
        ...
        if (len(line) < min_chars):
            ...
            print(f"  {line}\n")
            print(f"Too few characters.  This text has only {len(line)} characters, but expected at least {min_chars} characters.\n\n")
        elif (len(line) > max_chars):
            print(f"ERROR: in \"{filename}\", line {line_num}")
            print(f"  {line}\n")
            print(f"Too many characters.  This text has {len(line)} characters, but expected at most {max_chars} characters.\n\n")
